  ...
  static async getPlaceNameByCoordinate(latitude, longitude) {
    try {
      ...
      if (!json.features || json.features.length === 0) {
        console.warn('Tidak ada data lokasi yang ditemukan untuk koordinat ini.');
        return `${latitude}, ${longitude}`;
      }

      if (!json.features[0].place_name) {
        console.warn('Data lokasi tidak memiliki place_name');
        return `${latitude}, ${longitude}`;
      }
      ...
      if (place.length < 2) {
        return json.features[0].place_name || `${latitude}, ${longitude}`;
      }
